...
internal fun substituteInjectedBlocks(settings: CodeStyleSettings,
    ...
    val injectedBlocks = SmartList<Block>().apply {
        ...
        val fixedIndent = IndentImpl(Indent.Type.SPACES, false, settings.getIndentSize(SNTFileType.SNT), false, false)
        SNTInjectedLanguageBlockBuilder(settings, outerBLocks).addInjectedBlocks(this, injectionHost, wrap, alignment, fixedIndent)
    }
...
private class SNTInjectedLanguageBlockBuilder(settings: CodeStyleSettings, val outerBlocks: List<Block>)
    ...
    override fun createInjectedBlock(node: ASTNode,
        ...
        return SNTInjectedLanguageBlockWrapper(originalBlock, injectedToHost(trimmedRange), trimmedRange, outerBlocks, indent, SNTLanguage.INSTANCE)
    }
    ...
    private inner class SNTInjectedLanguageBlockWrapper(val original: Block,
        ...
        override fun toString(): String = "SNTInjectedLanguageBlockWrapper($original, $myRange," +
                ...
                for (block in original.subBlocks) {
                    myRange.intersection(block.textRange)?.let { blockRange ->
                        ...
                        fun createInnerWrapper(blockRangeInHost: TextRange, blockRange: TextRange, outerNodes: Collection<Block>) =
                            SNTInjectedLanguageBlockWrapper(block,
                                blockRangeInHost,
                                blockRange,
                                outerNodes,
    ...
    private fun Block.unwrap() = this.castSafelyTo<SNTInjectedLanguageBlockWrapper>()?.original ?: this
