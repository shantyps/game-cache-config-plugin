...
internal class SNTFormattingBlock(private val myContext: SNTFormattingContext, node: ASTNode) :
    ...
    private fun buildSubBlocks(context: SNTFormattingContext, node: ASTNode): List<Block> {
        val res: MutableList<Block> = SmartList()
        var subNode = node.firstChildNode
        while (subNode != null) {
            val subNodeType = PsiUtilCore.getElementType(subNode)
            if (SNTElementTypes.SPACE_ELEMENTS.contains(subNodeType)) {
                // just skip them (comment processed above)
            } else if (SNTElementTypes.SCALAR_QUOTED_STRING === subNodeType) {
                res.addAll(buildSubBlocks(context, subNode))
            } else if (SNTElementTypes.CONTAINERS.contains(subNodeType)) {
                res.addAll(
                ...
                res.add(SNTFormattingModelBuilder.createBlock(context, subNode))
            }
