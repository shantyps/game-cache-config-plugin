...
class SNTFormattingContext(val mySettings: CodeStyleSettings, private val myFile: PsiFile) {
    ...
    init {
        val custom = mySettings.getCustomSettings(SNTCodeStyleSettings::class.java)
        val common = mySettings.getCommonSettings(SNTLanguage.INSTANCE)
        mySpaceBuilder = SpacingBuilder(mySettings, SNTLanguage.INSTANCE)
            .between(YAMLTokenTypes.COLON, SNTElementTypes.KEY_VALUE_PAIR).lineBreakInCode()
            .between(YAMLTokenTypes.COLON, SNTElementTypes.SEQUENCE_ITEM).lineBreakInCode()
            .between(SNTElementTypes.ALIAS_NODE, YAMLTokenTypes.COLON).spaces(1)
    ...
    private fun getSpacingAfterSequenceMarker(child1: Block?, child2: Block): Spacing? {
        ...
        var indentSize = mySettings.getIndentSize(SNTFileType.SNT)
        if (indentSize < 2) {
            indentSize = 2
        }
        var spaces = 1
        var minLineFeeds = 0
        if (node2Type === SNTElementTypes.SEQUENCE_ITEM) {
        ...
        } else if (node2Type === SNTElementTypes.KEY_VALUE_PAIR) {
    ...
    fun computeAlignment(node: ASTNode): Alignment? {
        ...
        if (type === SNTElementTypes.SEQUENCE_ITEM) {
            if (node.treeParent.elementType === SNTElementTypes.ARRAY) {
        ...
        if (getValueAlignment == SNTCodeStyleSettings.ALIGN_ON_COLON) {
            if (type === YAMLTokenTypes.COLON) {
                return myChildValueAlignments[node.treeParent.treeParent]
            }
        } else if (getValueAlignment == SNTCodeStyleSettings.ALIGN_ON_VALUE) {
            if (SNTElementTypes.SCALAR_ITEMS.contains(type)) {
    ...
    fun computeBlockIndent(node: ASTNode): Indent? {
        ...
        return if (SNTElementTypes.DOCUMENT_BRACKETS.contains(nodeType)) {
            SAME_AS_PARENT_INDENT
        } else if (SNTElementTypes.BRACKETS.contains(nodeType)) {
            SAME_AS_INDENTED_ANCESTOR_INDENT
        } else if (SNTElementTypes.TEXT_SCALAR_ITEMS.contains(nodeType)) {
            if (grandParentType === SNTElementTypes.DOCUMENT) {
                return SAME_AS_PARENT_INDENT
            }
            if (grand2ParentType === SNTElementTypes.ARRAY || grand2ParentType === SNTElementTypes.HASH) {
                Indent.getContinuationWithoutFirstIndent()
            } else DIRECT_NORMAL_INDENT
        } else if (nodeType === SNTElementTypes.FILE) {
            SAME_AS_PARENT_INDENT
        } else if (SNTElementTypes.SCALAR_VALUES.contains(nodeType)) {
            DIRECT_NORMAL_INDENT
        } else if (nodeType === SNTElementTypes.SEQUENCE_ITEM) {
            computeSequenceItemIndent(node)
        } else if (nodeType === SNTElementTypes.KEY_VALUE_PAIR) {
            computeKeyValuePairIndent(node)
        } else {
            if (nodeType === YAMLTokenTypes.COMMENT) {
                if (parentType === SNTElementTypes.SEQUENCE) {
                    return computeSequenceItemIndent(node)
                }
                if (parentType === SNTElementTypes.MAPPING) {
                    return computeKeyValuePairIndent(node)
                }
            }
            if (SNTElementTypes.TOP_LEVEL.contains(parentType)) SAME_AS_PARENT_INDENT else null
        }
    }
    ...
    fun computeNewChildIndent(node: ASTNode): Indent? {
        return if (SNTElementTypes.TOP_LEVEL.contains(PsiUtilCore.getElementType(node))) SAME_AS_PARENT_INDENT else DIRECT_NORMAL_INDENT
    }

    fun isIncomplete(node: ASTNode): Boolean {
        val possiblyIncompleteValue =
            Predicate { value: YAMLValue? ->
                value == null || SNTElementTypes.INCOMPLETE_BLOCKS.contains(
                    PsiUtilCore.getElementType(value)
                )
            }
        if (PsiUtilCore.getElementType(node) === SNTElementTypes.KEY_VALUE_PAIR) {
            val value = (node.psi as YAMLKeyValue).value
            if (possiblyIncompleteValue.test(value)) {
                return true
            }
        } else if (PsiUtilCore.getElementType(node) === SNTElementTypes.SEQUENCE_ITEM) {
    ...
    private fun computeSequenceItemIndent(node: ASTNode): Indent {
        val parentType = PsiUtilCore.getElementType(node.treeParent)
        val grandParentType = if (parentType == null) null else PsiUtilCore.getElementType(node.treeParent.treeParent)
        val grandParentIsDocument = grandParentType === SNTElementTypes.DOCUMENT
        return if (parentType === SNTElementTypes.ARRAY) {
            Indent.getNormalIndent()
        } else if (grandParentType === SNTElementTypes.KEY_VALUE_PAIR) {
    ...
    companion object {
        ...
        private fun isAfterSequenceMarker(node: ASTNode?): Boolean {
            val nodes = StreamEx.iterate(node,
                ...
                .filter { n: ASTNode? ->
                    !SNTElementTypes.SPACE_ELEMENTS.contains(
                        n!!.elementType
                    )
                }
        ...
        private fun computeKeyValuePairIndent(node: ASTNode): Indent? {
            ...
            val grandParentIsDocument = grandParentType === SNTElementTypes.DOCUMENT
            return if (parentType === SNTElementTypes.HASH) {
                // {
                //   key: value
                // }
                Indent.getNormalIndent()
                ...
                // key: value
                SAME_AS_PARENT_INDENT
            } else if (parentType === SNTElementTypes.SEQUENCE_ITEM) {
                // [
                //   a: x,
                //   b: y
                // ]
        ...
        private fun getPreviousNonBlankNode(node: ASTNode): ASTNode? {
            ...
            do {
                ...
                if (!SNTElementTypes.BLANK_ELEMENTS.contains(PsiUtilCore.getElementType(node))) {
                    return node
                }
            } while (node != null)
            return null
