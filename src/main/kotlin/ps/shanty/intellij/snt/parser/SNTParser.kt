...
class SNTParser : PsiParser, LightPsiParser, YAMLTokenTypes {
    ...
    private fun parseDocument() {
        ...
        dropEolMarker()
        marker.done(SNTElementTypes.DOCUMENT)
    }
    ...
    private fun parseBlockNode(indent: Int, insideSequence: Boolean) {
        ...
        while (!eof() && (isJunk || !eolSeen || myIndent + getIndentBonus(insideSequence) >= indent)) {
            ...
            if (nodeType == null) {
                if (parsedTokenType === SNTElementTypes.SEQUENCE_ITEM) {
                    nodeType = SNTElementTypes.SEQUENCE
                } else if (parsedTokenType === SNTElementTypes.KEY_VALUE_PAIR) {
                    nodeType = SNTElementTypes.MAPPING
                } else if (numberOfItems > 1) {
                    nodeType = SNTElementTypes.COMPOUND_VALUE
                }
            }
    ...
    private fun includeBlockEmptyTail(indent: Int) {
        if (indent == 0) {
            ...
            while (isJunk) {
                if (tokenType === YAMLTokenTypes.EOL) {
                    if (!SNTElementTypes.BLANK_ELEMENTS.contains(myBuilder!!.lookAhead(1))) {
                        // do not include last \n into block
                        break
                    }
                }
    ...
    private fun parseSingleStatement(indent: Int, minIndent: Int): IElementType? {
        ...
        if (tokenType === YAMLTokenTypes.LBRACE) {
        ...
        } else if (SNTElementTypes.SCALAR_VALUES.contains(this.tokenType)) {
            ...
            if (this.tokenType === YAMLTokenTypes.ALIAS) {
                ...
                aliasMarker.done(SNTElementTypes.ALIAS_NODE)
                if (this.tokenType === YAMLTokenTypes.COLON) {
                    ...
                    marker.drop()
                    return SNTElementTypes.ALIAS_NODE
                }
            } else {
        ...
        while (tokenType === YAMLTokenTypes.TAG || tokenType === YAMLTokenTypes.AMPERSAND) {
            if (tokenType === YAMLTokenTypes.AMPERSAND) {
                ...
                if (tokenType === YAMLTokenTypes.ANCHOR) {
                    ...
                    anchorMarker.done(SNTElementTypes.ANCHOR_NODE)
                } else {
                    // Should be impossible now (because of lexer rules)
                    anchorMarker.drop()
                }
    ...
    private fun parseScalarValue(indent: Int): IElementType? {
        val tokenType = tokenType
        assert(SNTElementTypes.SCALAR_VALUES.contains(tokenType)) { "Scalar value expected!" }
    ...
    private fun parseQuotedString(): IElementType {
        advanceLexer()
        return SNTElementTypes.SCALAR_QUOTED_STRING
    }
    ...
    private fun parseMultiLineScalar(tokenType: IElementType?): IElementType {
        ...
        if (SNTElementTypes.EOL_ELEMENTS.contains(this.tokenType)) {
            advanceLexer()
        }
        ...
        return if (tokenType === YAMLTokenTypes.SCALAR_LIST) SNTElementTypes.SCALAR_LIST_VALUE else SNTElementTypes.SCALAR_TEXT_VALUE
    }
    ...
    private fun parseMultiLinePlainScalar(indent: Int): IElementType {
        ...
        return SNTElementTypes.SCALAR_PLAIN_VALUE
    }
    ...
    private fun parseExplicitKeyValue(indent: Int): IElementType {
        ...
        return SNTElementTypes.KEY_VALUE_PAIR
    }
    ...
    private fun parseSequenceItem(indent: Int): IElementType {
        ...
        rollBackToEol()
        return SNTElementTypes.SEQUENCE_ITEM
    }
    ...
    private fun parseArray(): IElementType {
        ...
        while (!eof()) {
            ...
            if (parsedElement != null) {
                marker.done(SNTElementTypes.SEQUENCE_ITEM)
            } else {
    ...
    private fun advanceLexer() {
        ...
        val eolElement = SNTElementTypes.EOL_ELEMENTS.contains(type)
